...
describe Puppet::Type.type(:jboss_datasource).provider(:jbosscli) do
  ...
  describe 'Result of self.instances()' do
    ...
    before :each do
      ...
      re = /.*bin\/jboss-cli.sh --timeout=50000 --connect --file=\/tmp\/jbosscli.* --controller=127.0.0.1:9990/
      expect(Puppet::Provider::Jbosscli).to receive(:last_execute_status).at_least(:once).and_return(status)
      expect(Puppet::Util::Execution).to receive(:execute).at_least(:once).with(re).and_return(xa_result, nonxa_result)
    end

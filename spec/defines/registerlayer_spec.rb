...
describe 'jboss::internal::module::registerlayer', :type => :define do
  ...
  it { is_expected.to contain_exec("jboss::module::layer::#{title}").with({
    :command => "/bin/awk -F'=' 'BEGIN {ins = 0} /^layers=/ { ins = ins + 1; print \$1=testlayer,\$2 } END {if(ins == 0) print \"layers=testlayer,base\"}' > #{dir}/#{product}-#{version}/modules/layers.conf",
    :unless  => "/bin/egrep -e '^layers=.*testlayer.*' #{dir}/#{product}-#{version}/modules/layers.conf",
    :user    => 'test-user'
    }) }
  it {
